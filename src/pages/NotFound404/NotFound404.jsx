import { Link } from 'react-router-dom'

export default function NotFound404() {
  return (
    <main className='flex h-screen w-full flex-col items-center justify-center bg-[#1A2238]'>
      <h1 className='text-9xl font-extrabold tracking-widest text-white'>404</h1>
      <div className='absolute rotate-12 rounded bg-orange px-2 text-sm'>Page Not Found</div>
      <button className='mt-5'>
        <Link
          to='/system/'
          className='active:text-orange-500 text-obg-orange group relative inline-block text-sm font-medium focus:outline-none focus:ring'
        >
          <span className='absolute inset-0 translate-x-0.5 translate-y-0.5 bg-orange transition-transform group-hover:translate-x-0 group-hover:translate-y-0' />
          <div className='relative block border border-current bg-slate-500 px-8 py-3'>
            <span>Go Home</span>
          </div>
        </Link>
      </button>
    </main>
  )
}
